...
require([
    ...
    apiGroups.forEach(function (groupEntry) {
      ...
      api.forEach(function (entry) {
        if (groupEntry === entry.group) {
          ...
          if (apiProject.url) {
            fields.article.url = fields.article.url.includes(apiProject.url)
              ? fields.article.url
  ...
  function _hasTypeInFields(fields) {
    ...
    $.each(fields, function (name) {
      if (
        _.any(fields[name], function (item) {
          return item.type;
        })
      )
        result = true;
  ...
  function loadGoogleFontCss() {
    ...
    var googleCss =
      "//fonts.googleapis.com/css?family=Source+Code+Pro|Source+Sans+Pro:400,600,700";
