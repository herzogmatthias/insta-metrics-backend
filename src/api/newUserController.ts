...
export const newUser = async (req: e.Request, res: e.Response) => {
    ...
    if(!validUsername) {
        res.status(400).json({text: 'Username does not exist', error: true} as Error)
        return;
    }
    const users = await UserRepository.getAllUsers() as any[]
    if(users.find(v => v.userName === req.params.username)) {
        res.status(400).json({text: 'Username already exists', error: true} as Error)
        return;
    }
    const ui = await UserInformation.InitAsync();
    const basicStats = await ui.getBasicStats(`https://www.instagram.com/${req.params.username}/`)
    let user: User = {
    ...
    res.json({error: false, text: 'New User added!'})
}